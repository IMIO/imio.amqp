...
class RabbitMQManager(object):
    ...
    def publish_message(self, exchange, routing_key, message):
        ...
        self._exec('publish', 'exchange={0}'.format(exchange),
    ...
    def declare_user(self, name, password):
        self._exec('declare', 'user', 'name={0}'.format(name),
                   'password={0}'.format(password), 'tags=')
        self.declared_users.append(name)

    def declare_permission(
        self, vhost, user, configure='.*', write='.*', read='.*',
    ):
        self._exec('declare', 'permission', 'vhost={0}'.format(vhost),
                   'user={0}'.format(user), 'configure={0}'.format(configure),
